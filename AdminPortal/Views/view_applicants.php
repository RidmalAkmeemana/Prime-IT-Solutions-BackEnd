...
$has_access_to_delete_applicant = false;
...
if (mysqli_num_rows($permission_query) > 0) {
	$has_access_to_delete_applicant = true;
}
...
<head>
	...
	<title><?php echo ($companyName); ?> - Applicants</title>
...
<body>
	...
	<div class="main-wrapper">
		...
		<!-- Delete Modal -->
		<div class="modal fade" id="Delete_Applicant" aria-hidden="true" role="dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-content p-2">
							<h4 class="modal-title">Delete <span id="deleteApplicantName"></span></h4>
							<p class="mb-4">Are you sure want to delete ?</p>

							<form method="POST" action="../../API/Admin/deleteApplicant.php" id="deleteApplicantForm" enctype="multipart/form-data">
	...
	<script>
		...
		$(document).ready(function() {
			...
			$.ajax({
				type: 'POST',
				url: '../../API/Admin/getAllApplicantData.php',
				dataType: 'json',
				success: function(data) {
					if (data.length > 0) {
						...
						$.each(data, function(index, row) {
							const canDelete = <?php echo $has_access_to_delete_applicant ? 'true' : 'false'; ?>;

							let actionButtons = `<div class="actions">`;

								actionButtons += `
								<a href="${row.Applicant_CV}" 
								download
								class="btn btn-sm bg-primary-light ms-1">
								<i class="fe fe-download"></i> Download CV
								</a>`;

							if (canDelete) {
								actionButtons += `
                                <a href="javascript:void(0);"
                                class="btn btn-sm bg-danger-light ms-1 delete-applicant-btn"
			...
			$(document).on('click', '.delete-applicant-btn', function() {
				...
				$('#Delete_Applicant input[name="Id"]').val(Id);
				$('#deleteApplicantName').text(applicantName);
				$('#Delete_Applicant').modal('show');
			});

			$('#deleteApplicantForm').submit(function(e) {
				e.preventDefault();
				$('#pageLoader').show();

				$.ajax({
					type: 'POST',
					url: '../../API/Admin/deleteApplicant.php',
					data: $(this).serialize(),
