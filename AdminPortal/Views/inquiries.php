...
// Check if user has access to deleteInquiries.php
$has_access_to_delete_inquiry = false;
$permission_query = mysqli_query($conn, "SELECT * FROM `tbl_backend_permissions` WHERE `Role` = '$user_status' AND `Backend_Id` = 230") or die(mysqli_error());
if (mysqli_num_rows($permission_query) > 0) {
	$has_access_to_delete_inquiry = true;
}
...
<body>
	...
	<div class="main-wrapper">
		...
		<!-- Delete Modal -->
		<div class="modal fade" id="Delete_Inquiry" aria-hidden="true" role="dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-content p-2">
							<h4 class="modal-title">Delete Review Id: <span id="deleteInquiry"></span></h4>
							<p class="mb-4">Are you sure want to delete ?</p>

							<form method="POST" action="../../API/Admin/deleteInquiries.php" id="deleteInquiryForm" enctype="multipart/form-data">
	...
	<script>
		...
		$(document).ready(function() {
			...
			$.ajax({
				type: 'POST',
				url: '../../API/Admin/getAllInquiriesData.php',
				dataType: 'json',
				success: function(data) {
					if (data.length > 0) {

						const canDelete = <?php echo $has_access_to_delete_inquiry ? 'true' : 'false'; ?>;
						...
						if (!canDelete) {
							table.column(6).visible(false); // Hide "Action"
						}
					}
				},
			...
			$(document).on('click', '.delete-addon-btn', function() {
				...
				$('#Delete_Inquiry input[name="Id"]').val(Id);
				$('#deleteInquiry').text(Id);
				$('#Delete_Inquiry').modal('show');
			});

			$('#deleteInquiryForm').submit(function(e) {
				e.preventDefault();
				$('#pageLoader').show();

				$.ajax({
					type: 'POST',
					url: '../../API/Admin/deleteInquiries.php',
					data: $(this).serialize(),
